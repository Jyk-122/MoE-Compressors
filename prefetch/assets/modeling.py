#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
#           This file was automatically generated from src/transformers/models/openPXX_v2/modular_openPXX_v2.py.
#               Do NOT edit this file manually as any edits will be overwritten by the generation of
#             the file from the modular. If any change should be done, please apply the change to the
#                          modular_openPXX_v2.py file directly. One of our CI enforces this.
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
# coding=utf-8
...
import transformers
from .configuration_openPXX_v2 import OpenPXXV2Config


class mHCModule(nn.Module):
    def __init__(
        self,
        config: OpenPXXV2Config,
    ...
    def hc_split_sinkhorn_torch(self, weight):
        ...
        # h_pre = torch.sigmoid(h_pre * alpha_pre + beta_pre)
...
class OpenPXXV2RMSNorm(nn.Module):
    def __init__(self, hidden_size, eps: float = 1e-6) -> None:
        """
        OpenPXXV2RMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class OpenPXXV2RotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: OpenPXXV2Config, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: OpenPXXV2Config | None = None,
...
class OpenPXXV2Attention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: OpenPXXV2Config, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.layer_type = config.layer_types[layer_idx] \
                        if (hasattr(config, "layer_types") and config.layer_types is not None) else None
        ...
        self.qkv_proj = nn.Linear(
            config.hidden_size, self.qkv_size, bias=config.attention_bias
        )
        
        self.k_layernorm = OpenPXXV2RMSNorm(hidden_size=self.head_dim, eps=config.rms_norm_eps)
        ...
        if self.attn_groupnorm:
            self.groupnorm = OpenPXXV2RMSNorm(hidden_size=self.head_dim, eps=config.rms_norm_eps)
...
class OpenPXXV2MLAAttention(nn.Module):
    def __init__(self, config: OpenPXXV2Config, layer_idx: int):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        ...
        if self.q_lora_rank is None:
            ...
            self.q_a_layernorm = OpenPXXV2RMSNorm(config.q_lora_rank)
        ...
        self.kv_a_layernorm = OpenPXXV2RMSNorm(self.kv_lora_rank)
...
@use_experts_implementation
class OpenPXXV2Experts(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            expert_mask = torch.nn.functional.one_hot(top_k_index, num_classes=self.num_experts)  # batch_size, tokens, experts
            expert_mask = expert_mask.permute(2, 1, 0)  # experts, tokens, batch_size
            expert_hit = torch.greater(expert_mask.sum(dim=(-1, -2)), 0).nonzero()  #  在所有batch的所有tokens中，有被激活的专家有哪些

        for expert_idx in expert_hit:
            expert_idx = expert_idx[0]
            if expert_idx == self.num_experts:
                continue
            top_k_pos, token_idx = torch.where(expert_mask[expert_idx])  # 转件expert_idx激活的token的idx、以及所在batch的idx
...
class OpenPXXV2TopkRouter(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class OpenPXXV2SparseMoeBlock(nn.Module):
    """
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.experts = OpenPXXV2Experts(config)
        self.gate = OpenPXXV2TopkRouter(config)
        self.shared_experts = OpenPXXV2MLP(
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = hidden_states + self.shared_experts(residuals)  # 残差需要过一下共享专家
        return hidden_states


class OpenPXXV2DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: OpenPXXV2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        if config.use_mla:
            self.self_attn = OpenPXXV2MLAAttention(config=config, layer_idx=layer_idx)
        else:
            self.self_attn = OpenPXXV2Attention(config=config, layer_idx=layer_idx)
        self.attention_type = config.layer_types[layer_idx] \
                        if (hasattr(config, "layer_types") and config.layer_types is not None) else None

        if layer_idx >= config.first_k_dense_replace:
            self.mlp = OpenPXXV2SparseMoeBlock(config)  # 前馈网络是moe
        else:
            self.mlp = OpenPXXV2MLP(config)  # 前馈网络是mlp

        self.input_layernorm = OpenPXXV2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.pre_mlp_layernorm = OpenPXXV2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)

        self.sandwich_norm = getattr(config, "sandwich_norm", False)
        if self.sandwich_norm:
            self.pre_mlp_layernorm = OpenPXXV2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.post_mlp_layernorm = OpenPXXV2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        ...
        if self.has_block_post_layernorm:
            self.block_post_layernorm = OpenPXXV2RMSNorm(block_post_layernorm_hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class OpenPXXV2PreTrainedModel(PreTrainedModel):
    config: OpenPXXV2Config
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["OpenPXXV2DecoderLayer"]
    ...
    _can_record_outputs = {
        "hidden_states": OpenPXXV2DecoderLayer,
        "attentions": OpenPXXV2Attention,
    }
    _keys_to_ignore_on_load_unexpected = [r"model\.layers\.44.*"]


@auto_docstring
class OpenPXXV2Model(OpenPXXV2PreTrainedModel):
    def __init__(self, config: OpenPXXV2Config):
        ...
        self.layers = nn.ModuleList(
            [OpenPXXV2DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = OpenPXXV2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = OpenPXXV2RotaryEmbedding(config=config)
...
@auto_docstring
class OpenPXXV2ForCausalLM(OpenPXXV2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = OpenPXXV2Model(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, OpenPXXV2ForCausalLM

        >>> model = OpenPXXV2ForCausalLM.from_pretrained("meta-open_PXX_v2/OpenPXXV2-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-open_PXX_v2/OpenPXXV2-2-7b-hf")
...
__all__ = ["OpenPXXV2PreTrainedModel", "OpenPXXV2Model", "OpenPXXV2ForCausalLM"]