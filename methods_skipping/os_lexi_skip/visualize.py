...
def calib_with_existing_model(
    ...
    """使用已有的模型进行 calib，避免重复加载"""
    import types
    import logging
    from safetensors.torch import save_file
    from methods_skipping.os_lexi_skip.model_qwen3_moe import _resolve_layer_topk, _get_moe_layers
    ...
    # 恢复原始的 forward 方法，因为我们要复用这个模型
    # 需要重新加载原始模型来恢复，或者保存原始的 forward 方法
    # 这里我们选择重新加载模型，避免复杂的 hook 管理
    return None
...
def main():
    ...
    num_moe_layers = len(moe_layers)
    top_k = orig_model.config.num_experts_per_tok
    num_experts = orig_model.config.num_experts
    
    if args.layers is None:
        args.layers = [idx for idx, _ in moe_layers]
    
    for config_idx, (config_name, layer_topk) in enumerate(configs.items()):
        ...
        if not adapter_dir.exists():
            ...
            # calib 之后重新加载原始模型，因为我们修改了它的 forward 方法
            del orig_model
            torch.cuda.empty_cache()
            print("Reloading original model after calibration...")
            orig_model = AutoModelForCausalLM.from_pretrained(
                args.model,
                torch_dtype=torch.bfloat16,
                device_map=device,
            ...
            # 重新收集原始 hidden states（虽然可能不需要，但为了安全）
            # 实际上我们已经保存了 orig_hiddens，所以可以跳过
